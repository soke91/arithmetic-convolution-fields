...
cell mean departs from the band mean cannot be asked without a
reference scale for that contrast, and the obvious one --- the sample
deviation over the square root of the cell size --- turns out to be
wrong here by a factor that grows with the band.

Trying to measure that discrepancy produced an exact statement rather
than an estimate. Under the null replacing the $\mu(v)$ on their own
support by independent Rademacher signs, the variance of any cell
contrast is a doubly centred quadratic form in the field's covariance
kernel and reduces to three convolutions
(Lemma~\ref{lem:cellmom}); a uniformly random cell of the same size
recovers the finite-population formula exactly
(Lemma~\ref{lem:permfloor}). That $\mathrm{sd}/\sqrt{n_c}$ fails for
correlated observations is standard. What is not is that here the
kernel is computable in closed form, so the correct scale can be
written down rather than estimated, and that the divisibility cells
align with it while a random cell does not.

Measured over eight octaves to $1.6\cdot10^{7}$, the exact null
standard deviation shrinks by $1.21$ where a count predicts $11.3$, and
a label permutation preserving every cell size collapses it by factors
$3.8$ to $105$. For its size we offer a candidate descriptor found the
same way: the excess $D_c$ of the shift's singular series over
same-cell pairs, whose main term is a rational number and carries no
fitted parameter. \textbf{Two things are claimed for it and they carry
different weight.} That the residue structure determines $D_c$ was
written down for a second, differently shaped partition and its
predictions committed before that partition was measured; the exact
measurement falls inside the registered interval at every cell, and
that transfer is the evidence offered. That $D_c$ in turn determines
the sign-null floor rests on a correlation of $0.9805$ across the six
cells of the first partition, computed after the fact and not
transferred: the second partition was not measured for its floor.
Descriptor and mechanism are therefore not on the same footing here,
and the second word is not used for the second link. One rule
registered on the first partition was refuted.

Every registered rule is reported with its verdict, including the
thirteen the runs did not meet and the one recorded void. No
distributional law for $Z$ is asserted, no Goldbach consequence is
...
\textbf{That bar is an upper bound and not the precision.}
$1/\sqrt{2(n-1)}$ is the relative standard error of a sample standard
deviation in the Gaussian case; in general it carries the fourth
moment, and here the fourth moment is available in closed form, since
each contrast is a weighted Rademacher sum $\sum_v a_v\varepsilon_v$
with $\mathbb E S^4=3(\sum a^2)^2-2\sum a^4$. The kurtosis is therefore
$3-2\sum a^4/(\sum a^2)^2$, below the Gaussian $3$ at every cell, and
the precision is $1/\sqrt{2(n-1)}$ times
$\sqrt{1-\sum a^4/(\sum a^2)^2}$ --- smaller than the bar used, and
\emph{different at each cell}, since the weights are. The direction
matters for the sentence above: a cell-specific bar is tighter, so
``only the cell of three clears'' is a statement about the common bar
and not about the cells. What the common bar does support is the
weaker reading, that the deviations are small on the scale the draw
count alone can guarantee. The cell-wise fourth moments are not
computed here. That is one deviation
