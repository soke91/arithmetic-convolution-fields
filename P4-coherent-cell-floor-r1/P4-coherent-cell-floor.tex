...
% CLAIM:none
No Gaussian or limiting distributional law for $Z$ and no Goldbach consequence is asserted,
...
derived.} The exact doubly centred singular-series analogue of the
...
\]
After division by the $m^2\ell^2$ ordered pairs of a cell, this is
...
the pairs with $d\mid h$ number $\ell^2/d+O(\ell)$ --- the count is a
...
Neither varies by cell. The conclusion needs no ordering, only that
...
computed.} An independent reading of the construction reaches both
...
\noindent in the directory \texttt{P4-coherent-cell-floor-r1/}, which
holds this revision. The directory \texttt{P4-coherent-cell-floor/}
beside it holds the version sent to the journal and is frozen, so a
reader comparing the two is reading a revision against what was
actually submitted rather than against a working copy. That address is
mutable, and this revision is not in a tagged release: the tagged
\texttt{v1.0.1}, commit \texttt{b2f365d}, predates the revision
directory, and its second-cell scripts and outputs are not the ones
read here. What pins the bytes travels with them instead ---
\texttt{PACKET.json} in that directory carries a SHA-256 for each of
the thirty files in \texttt{code/} and \texttt{results/}, so a figure
can be checked against the exact source that produced it without
resolving a tag. The pin is on those and not on this document, which
cannot cite a hash that covers it. Each entry
...
its verdict, including the thirteen the ten named runs did not meet, the
...
Six of the ten named runs record a rule their own output did not meet.
...
are registered in all. The fourteen are named here because a
...
one of the ten named runs. Nothing above rests on that sampled rule.
