...
%
%  NOT the canonical manuscript.  This is the unsorted version:
%  propositions and lemmas stand beside notes, an observation and the
%  measurements here, at mixed strength.  P4-coherent-cell-floor.tex is
%  the canonical one.  This file is kept because the scope section
%  "What is not claimed" lives in it.
%
...
into cells indexed by how many of $3,5,7,11,13$ divide $N$.
% CLAIM:C5
Whether a
cell mean departs from the band mean needs a reference scale, and the
count-based one misses it by a factor growing with the band.
...
three computable terms built from finite correlations and weighted
inner products: the exact null scale is written down, not estimated.
% CLAIM:C2
For a uniformly random same-size cell, the
permutation average of the sign-null variance equals the
...
and none of ten sampled label permutations preserving every cell size
reproduced the floor; their collapse factors range from $3.8$ to $105$.
...
no fitted parameter.
% CLAIM:C3
For a second, differently shaped partition, the exhaustive measurement
at the three scored depths falls inside the interval computed from the
residue main term. The fourth
depth is an algebraic consistency check, not a scored interval.
% CLAIM:C4
At this band, $D_c$ tracks the null floor with correlation $0.9805$
across six cells; this post hoc association was not transferred.
% CLAIM:none
Every
registered rule is reported with its verdict, refuted ones included.
% CLAIM:none
No Gaussian or limiting distributional law for $Z$ and no Goldbach consequence are asserted,
...
$3,5,7,11,13$ dividing $N$. \textbf{Nothing below derives that set, and
Measurement~\ref{meas:sweep} reports what happens on eighteen others.}
Over the octave $(2\cdot10^6,4\cdot10^6]$ these five give six depths
with $67$ even $N$ at the deepest, against $4$ if $17$ is adjoined and
$866$, with one depth fewer, if $13$ is dropped. The question is whether
$m_c-\overline m$, the difference between the cell mean of $Z$ and the
band mean, is larger than the sign null admits. \emph{Chance} is not
the word for it and is not used below: that null replaces $\mu$ on its
own support by independent signs and so discards its multiplicativity,
and Note~\ref{note:floorsignal} reports an ensemble that keeps the
multiplicativity as the wider of the two, with no transfer between them
established here. Every standardised figure below is against the sign
null.
...
for a uniformly random same-size cell, the permutation average of the
sign-null variance equals the finite-population formula exactly; and
that the size of the effect is
...
exactly under the sign null, as three explicit terms built from finite
correlations and weighted inner products.
% CLAIM:C5
...
read against a finite range, not a proved rate; what is established over
the measured range is that count-based scaling does not describe the
arithmetic-cell floor.
% CLAIM:C6
...
% CLAIM:C6
\item Measurement~\ref{meas:placebo} runs it, and finds that neither the
cell effect nor the arithmetic-cell floor is reproduced under the
placebo; the floor itself collapses, so it is a second measurement of
the same correspondence.
% CLAIM:C7,C4
\item Proposition~\ref{prop:scaleinv} shows the quantity offered for
...
profile at four of the six depths with no fitted parameter, the other
two accounted for without one --- which both
...
a \emph{second} cell shape, computing its profile from the residues alone
and then evaluating the finite pair average exhaustively. The computed
intervals contain the exact values where the rules could
score them, and the small
...
\item \textbf{No Gaussian or limiting distributional law for $Z$ is asserted.}
\item \textbf{No consequence for the Goldbach problem.} Nothing
measured here bounds $C(N)$ in either direction.
\item \textbf{That $D_c$ predicts the floor's size is measured, not
derived.} The off-diagonal three-term singular-series analogue of the
variance is $B_c$, defined in \S\ref{sec:size}; $D_c$ itself is the
two-term descriptor in \eqref{eq:Dc}. Measurement~\ref{meas:Dc}
shows that $D_c$ and $B_c$ are close in the measured band because the
cross average of the singular series is close to its band average. A
constant ratio between the variance and $B_c$ would follow if the two
kernels differed across the band by a factor the centring leaves alone.
No such statement is proved here. What is measured is the
...
$D_c$ moves by a factor $68$, and the correlation $0.9805$ carries an exact enumeration over
all $720$ relabellings as its control. That is one octave read across depths.
...
at depth~$1$; an exponent is fitted
directly instead.
...
\[
  ...
  \qquad
  Q_{cd}\;=\;\sum_{1\le v<N_{\max}}\mu^2(v)\,u_c(v)\,u_d(v),
\]
where $N_{\max}$ is the largest element of the band and
$\Lambda(k)=0$ for $k\le0$, so that the inner sum runs over the $N$ of
$c$ that exceed $v$ and the outer one over the squarefree $v$ below the
band's top. Then, when the $\mu(v)$ on the surviving support are
replaced by independent signs,
\begin{equation}\label{eq:cellmom}
  ...
  \;+\;\frac{Q_{aa}}{n^{2}},
\end{equation}
where $a$ denotes the whole band. The quantities $u_c$ are finite
correlations of explicit sequences, and each $Q_{cd}$ is then an
explicit weighted inner product; the identity is exact and no
simulation is involved. Its numerical evaluation is a separate matter
--- these finite operations are carried out in double precision
...
For later use, write
\[
  \mathrm{se}_c:=\sqrt{\mathrm{Var}_\varepsilon(m_c-\overline m)},
  \qquad
  z_c:=\frac{m_c-\overline m}{\mathrm{se}_c}.
\]
Throughout, the \emph{null standard deviation} of a cell contrast means
$\mathrm{se}_c$: the exact standard deviation of $m_c-\overline m$
under the Rademacher null, at the observed cell sizes and the observed
support of $\mu^2$. It is a scale, not a tail bound. The lemma fixes the
scale exactly, but no Gaussian, limiting, or other calibrated tail law
is invoked for $m_c-\overline m$; no probability is attached below to
a multiple of $\mathrm{se}_c$ merely because it is such a multiple.
...
\label{meas:mc}
The lemma does not need simulation, but simulation is an independent
numerical check on its closed form, and every normalised cell mean
below is divided by that form. Run in the band $(10^5,2\cdot10^5]$ with
$2000$ draws:
\[
...
$n_c^{-1/2}$. On average over the $v$ that carry weight, about
$n_c/\log N$ of the terms of $u_c(v)$ are nonzero, each of size
$\log N/\sqrt V$, so $u_c(v)\approx n_c/\sqrt V$ and
\[
  \frac{Q_{cc}}{n_c^2}
  \;\approx\;\frac{\sum_v\mu^2(v)}{V}
  \;\asymp\;\frac{1}{\log N},
\]
independently of $n_c$. \emph{At a fixed $v$ this is false}: the band
is even, so an even $v$ leaves $N-v$ even and $\Lambda(N-v)$ supported
on powers of $2$ alone, and the cell's own primes impose further local
obstructions. What the display uses is the sum over $v$, where those
obstructions are the local densities being averaged; the heuristic is
offered at that level and not at a single $v$. By
Note~\ref{note:threeterms} the variance
...
is constant across the eight fitted points --- its spread over them is
$0$ at depths~$0$ and~$1$ --- the least-squares slope of
$\log(Q_{cc}n_c^{-2})$ against $\log N$ is $-2b$. Writing $s_{\log}$
for the least-squares slope of $\log\log N$ against $\log N$ over the
same eight points, the slope $g$ of $\log(Q_{cc}n_c^{-2}\log N)$
satisfies
\[
  g=-2b+s_{\log},\qquad b=\tfrac12\bigl(s_{\log}-g\bigr),
\]
and \textbf{this relation between the fitted slopes is exact} under
that constancy, because a least-squares slope is linear in the data.
\textbf{What is not exact is replacing $s_{\log}$ by
$1/\langle\log N\rangle$}: $1/\log N$ is the derivative of $\log\log N$
and not its least-squares slope over a finite range. The two are kept
distinct below, and the difference is immaterial to the comparison the
next measurement makes.
Nothing in this paragraph should be read as the drift confirming the
exponent.

What the residual of that relation does read is the constancy of the
...
alongside the break and is not offered as its cause. An exponent is
fitted directly at each depth, and that measurement, not the form, is
...
The slow decay is measured directly --- the decay and not the
heuristic's form, which Observation~\ref{obs:coh} finds loose at every
depth. Fitting $\mathrm{se}_c\propto N^{-b}$ to
the exact floor across eight octaves gives
$b=0.039451,\,0.039671,\,0.039388$ at depths $2,1,0$, against the
$0.5$ that a count would give and against $s_{\log}/2=0.0363195$, half
the least-squares slope of $\log\log N$ against $\log N$ over the same
eight octave midpoints. The local approximation
$1/(2\langle\log N\rangle)=0.036038$, with $\langle\log N\rangle$ the
mean of $\log N$ over those midpoints, gives the same comparison and is
the quantity a registered rule of this run was written against.
Nothing turns on the third decimal: the point is that $0.039$ is near
the logarithmic scale and nowhere near $0.5$
...
statistic under $\ell\circ\pi$ is its law under a uniformly random
fixed-count relabelling chosen independently of $Z$; and a statistic
...
both invariant, and the joint distribution of $(\ell\circ\pi,Z)$ is that of a uniform
fixed-count relabelling, with the random relabelling independent of
$Z$. That observation gives both halves: the
...
\begin{measurement}[the cell contrasts are not zero]\label{meas:notzero}
Against the floor of Lemma~\ref{lem:cellmom}, computed from its closed
form, the cell contrasts $m_c-\overline m$ of the field
\eqref{eq:field} are not all zero. Over
...
\]
No multiple-comparison threshold is asserted for these. The finite
Rademacher sign ensemble does define a joint law, but this paper does
not calibrate the tail of $\max_c|z_c|$ under that law. The numbers are
therefore reported only as a maximum over cells of standardised
deviations against the closed-form null floor. The signal is carried by the deep cells:
at the top octave depths $3,4,5$ sit at $z=-1.55,\,-4.46,\,-9.06$
while depths $0,1,2$ sit at $+0.04,\,+0.69,\,-0.06$
(\texttt{lab\_cell\_floor.py}).
\end{measurement}

\begin{measurement}[the placebo reproduces neither the effect nor the
floor]\label{meas:placebo}
...
labelling and $+0.0042$ on average under permutation. None of the ten
sampled permutations reproduced either true value --- which is a
statement about the sample and not about the permutation law --- so by
the second half of
...
(\texttt{audit\_cn\_coin\_deep.py}). The run prints that ratio to five
places and quotes no sampling uncertainty for it; at $512$ draws only
the leading two figures are earned, and nothing below rests on more.
That measurement rests on one band, one class family and one
statistic, and its own registration says so: it is the second
moment $E[Z^2]$ --- the run writes $G$ for $Z$ --- on the classes cut by $3,5,7$ over
...
\begin{array}{r|cccccc}
 ...
 m_c-\overline m & +0.01325 & +0.05191 & -0.03432 & -0.58002
   & -1.99506 & -4.81664\\
 \bigl|m_c-\overline m\bigr|\big/\sigma_c
...
\[
  ...
  \qquad
  B_c:=E_{\mathrm{same},c}[\SS_2]-2E_{c,a}[\SS_2]
       +E_{\mathrm{all}}[\SS_2].
\]
Thus $B_c$, not $D_c$, is the off-diagonal singular-series quantity
with the same three-part centring as \eqref{eq:JKJ}. The two-term
$D_c$ is retained as the descriptor used by the measurements below;
Measurement~\ref{meas:Dc} quantifies how close it is to the doubly
centred convention in the measured band.
...
the depth labels exactly one reaches $|r|=0.9805$, against a median of
$0.3475$ and a $95$th percentile of $0.8084$ over that set. Those are
order statistics of the $720$ relabellings and not quantiles of a law:
the depths are deterministic, no exchangeability of the labels is
assumed, and what the enumeration measures is how much of the agreement
is carried by which cell gets which number rather than by the shape of
either row alone. The
...
comparison to make. The exact doubly centred comparison is between two kernels:
\eqref{eq:JKJ} is the doubly centred mean
$n_c^{-2}\sum J_cJ_cK$, while $B_c$ is the corresponding off-diagonal
doubly centred mean with $K$ replaced by the singular series of the
shift. The descriptor $D_c$ in \eqref{eq:Dc} omits the cross-centering
term and is therefore not literally the same functional. The centring is not
...
(Measurement~\ref{meas:exponent} fits it a nonzero exponent), while
Proposition~\ref{prop:scaleinv} shows that $D_c$ is asymptotically
scale-free: $D_c$ is therefore not all of what fixes the
floor's size.
\end{measurement}
...
interval of length $B$, and put $\ell=B/2Q$, so that each of the $Q$
\emph{even} residue classes modulo $2Q$ meets $a$ in $\ell+O(1)$ terms
and each odd class meets it in none. Let $R_c$ be a
nonempty set of $m$ even residue classes modulo $2Q$ and
...
depends on $B$ or on the position of the interval, and the parameter
set $\{3,5,7,11,13\}$ enters only through $Q$, fixed throughout. The
first constant is absolute in the sense of being bounded over the cells
and not of being one number for all of them: the rounding residue
isolated in the proof carries a constant that varies by two orders of
magnitude between depth~$0$ and depth~$4$, and it is the finiteness of
the field --- $Q$ fixed, hence finitely many cells --- that makes the
supremum a constant at all. Nothing below needs the stronger reading. The $\eta$-dependence is not uniform as
...
$B\to\infty$ with $R_c$ held fixed. Fix a family of bands $I_B$ of
length $B$ --- here $I_B=(B,2B]$, the octaves the measurements use ---
and write $D_c(B)$ for the value of \eqref{eq:Dc} on $I_B$; a length
alone would not name one. Define the one-step exponent
...
class in $\ell+O(1)$ terms, and it is that count, not the cell's
...
$N-h\in c$, and membership of $c$ is fixed by $N\bmod 2Q$, so the count
reads $h\bmod 2Q$ as well: if $3\mid h$ the shift preserves divisibility
by $3$ and otherwise it does not. It does not, however, factor. Write
$f_c(h)=\#\bigl(R_c\cap(R_c+h)\bigr)$ for the number of classes that a
shift by $h$ keeps inside $R_c$. The $N$ of one such class that are
admissible fill an interval of length $B-|h|$, which the class meets
$(B-|h|)/2Q$ times up to the rounding at its two ends, so
\[
  W_c(h)\;=\;\#\{N\in c:\ N-h\in c\}
  \;=\;f_c(h)\,\frac{B-|h|}{2Q}\;+\;\rho_c(h),
  \qquad
  |\rho_c(h)|\;\le\;f_c(h)\;\le\;m .
\]
The residue $\rho_c$ is that rounding summed over the surviving classes.
It is additive rather than a further factor, and it is bounded
independently of $B$.

The unweighted ratio is $O(1/\ell)$, but the pair average carries
$\rho_c(h)$ against $T(h)$, and $T$ is not uniformly bounded. The
weighted residue is instead absorbed by the already displayed
$O_\eta(B^{\eta-1})$ term. Indeed
\[
  T(h)=\sum_{d\mid h}g(d)\leq\tau(|h|),\qquad
  \sum_{0<|h|<B}|\rho_c(h)|T(h)
  \leq m\sum_{0<|h|<B}\tau(|h|)=O_\eta(mB^{1+\eta}).
\]
After division by the $n_c(n_c-1)=m^2\ell^2(1+O(1/\ell))$ ordered pairs
of a cell, this is
$O_\eta(B^{\eta-1})$ uniformly in the finitely many cells (with $Q$
fixed). Thus no unweighted estimate is used after multiplication by
$T$.

What the argument uses is therefore the leading product, with the
weighted $\rho_c$ contribution carried into $O_\eta(B^{\eta-1})$. Its
second factor is an envelope in $|h|$:
normalised to sum $1$ it is $(B-|h|)/Z$ with $Z=\sum_h(B-|h|)$, so
consecutive values differ by $2/Z$ and there are $O(B)$ of those steps,
whence total variation $O(1/B)$ --- with constant $4$ when $h$ runs over
the even integers, since then $Z=B^2/2+O(B)$. Lemma~\ref{lem:envtv}
applied to it gives $P(d\mid h)=1/d+O(1/B)$ uniformly in $d\le B$. The
mixture over classes is what the pattern weights supply. Every cell's
pair law is such a mixture, whence
\[
...
The three parts of $\mathcal E$ are derived in the paragraphs that
follow: the first from counting a cell against its residue classes, the
second by summing that error against $g$, and the third from the weighted
rounding residue and the two truncations of the divisor sum. They are
stated here and justified there rather than the other way round, so that
the shape of the answer is visible before the bookkeeping.

All three parts of $\mathcal E$ have to be said. None of their
\emph{rates} depends on the cell: $\ell=B/2Q$ is a band parameter, the
same at depth~$0$ and at depth~$5$. The constants in front of them are
the separate question answered above for the first --- bounded over the
cells because $Q$ is fixed, and not one number for all of them. A cell-dependent bound would have to be an $O(1/n_c)$,
and the counting below does not support one.
At the level of the displayed powers, the divisor-sampling bound is
$B^{\eta}$ times the first, while
$B^{\eta}/\ell=2Q\,B^{\eta-1}$, so the latter two bounds have the
same asymptotic order for fixed $Q$. Their finite-band numerical sizes
cannot be ordered without the unnamed implied constants, and the
conclusion does not require such an ordering. The $O(1/B)$ above is the error of the
...
meeting the band in $\ell+O(1)$ terms with $\ell=B/2Q$, so that
$m\ell=n_c+O(m)$; only the
deepest cell of this field is a single class. Over one pair of classes
the off-diagonal pairs with $d\mid h$ number $\ell^2/d+O(\ell)$ --- the count is a
tent summed along a progression, and its uniformity in $d$ is the
paragraph on uniformity below --- so over the $m^2$
...
$O_\eta(B^{\eta}/\ell)$; $\ell$ is a fixed fraction of $B$ --- namely
$B/2Q$ with $Q$ fixed, so that $B^{\eta}/\ell=2Q\,B^{\eta-1}$ as
recorded above --- so this is $O_\eta(B^{\eta-1})$ again.
...
resolution seen twice and not two errors. Both are bounds rather than errors, and their finite-band numerical
ordering is not supplied because the implied constants are unnamed.
Neither rate varies by cell. The conclusion needs no ordering, only that
both go to zero, and $\ell$ grows with the band.
...
$[0,\ell)^2$ with $i\ne j$ and $i-j$ in a fixed class modulo $d$ number
...
divide out identically against $n_c^2=m^2\ell^2(1+O(1/\ell))$: the same relative
...
no fitted parameter in it, and a rational one: the same
...
\begin{array}{r|cccccc}
 ...
 \text{measured} & 0.302138 & 0.046766 & 0.412504 & 1.052129
   & 1.946109 & 3.171298\\
 \text{residual}/\mathrm{se}_{\mathrm{pair}} & 2.0 & 0.6 & 0.5 & 0.2 & 0.1 & 260.8
\end{array}
\]
Six numbers are predicted from none, across cells spanning a factor of
$68$. Here $\mathrm{se}_{\mathrm{pair}}$ is the sampling error of the
pair average that the measured row carries, and not the null floor
$\mathrm{se}_c$ of \S\ref{sec:floor}: at depth~$0$ the first is
$0.001358$ and the second $0.12400$
(\texttt{lab\_cell\_singular.py}). \emph{Four of the six sit inside two of those sampling
errors of the measured value}, depth~$0$ sits at $2.0$, and the sixth
is depth~$5$ at
...
the observed four in two of them --- a fraction $2/720$ of the
enumerated set, read as in Measurement~\ref{meas:Dc}'s control and not
as a $p$-value under a law that produced the depths; the six
predictions span a factor
of $70$, so a wrong pairing is refused by size alone, and what the
enumeration shows is that the agreement is carried by which cell gets
which number.
...
a failure of scale invariance: it is a computed component of the
proposition's own error.} An independent reading of the construction reaches both
figures without this computation, putting the residual at $1.35$ per
cent and claiming only the order for the spread. The packet does not
assert which was carried out first.
...
\begin{array}{r|cccccc}
 ...
 \text{predicted} & 0.299361 & 0.045996 & 0.411876 & 1.052351
   & 1.946236 & 3.169984\\
 \text{residual}/\mathrm{se}_{\mathrm{pair}} & -1.13 & +0.36 & +0.45 & +1.07 & +1.16
   & -0.43
\end{array}
\]
...
\begin{measurement}[a second cell shape, computed from residues alone]\label{meas:secondcell}
...
second partition can be \emph{computed without reference to its own
measurement}, which the first can no longer be given.
...
$K\,E_{\mathrm{same},c}[A_S]-2$ are computed from the residues alone
(\texttt{lab\_secondcell\_predict.py}), taking no input from the
measured partition. The measurement (\texttt{lab\_secondcell2.py},
registered in \texttt{lab\_secondcell2\_predict.py}) is exhaustive: a cell
is a set of
...
measurement are the same finite sum, as the registration records.
...
and the registration file records both that lean and that it was not
decisive. The exhaustive finite-sum value is inside the interval. So that is one pass
against a stated lean --- not two, and not against a prediction of
failure: the registration file says ``leans, not decisive'' of the first and
...
At depth~$0$ the deficit, the covariance and the pattern weight --- in
that order, which is not the table's --- are $5.68$, $4.36$ and $1.95$
per cent of $1/\ell=1.05\cdot10^{-4}$, each first divided by
$2C_2E_c[A_S]$. That constant is the deficit's own factor in the four-term
decomposition above, so dividing returns $\Delta_c$ there; the other two
carry $K$ and $2C_2$ instead, and are divided by the same constant only
to put all three on one scale. Read straight off the table's entries
the same three are $12.6$, $9.7$ and $4.3$ per cent. Complete
...
deficit at $4.03$ per cent of $1/\ell$ on that same scale.
The proposition's error term carries an unspecified implied constant
and a divisor sum above three, as the discussion after its proof
records. So the asymptotic statement is not quantitative enough, at
this finite band, to turn these percentages into a certified fraction
of its bound: they are reported as a comparison with the $1/\ell$ that
appears in the proof, and not as the fraction of the bound the
measurement realises.
...
\begin{measurement}[the closed form on nineteen cell shapes]\label{meas:sweep}
Two shapes are two shapes. The same computation runs for any set $S$ of
odd primes, so it was run on nineteen: every pair and triple from
$\{3,5,7,11,13\}$ that the band resolves, four quadruples,
$\{3,5,7,11,13\}$ itself and that set with $17$ adjoined
(\texttt{lab\_cellset\_sweep.py}). Two rules were registered before the
run --- $R_1$, that $|\text{main}-\text{exact}|<|\text{exact}|$, and
$R_2$, that it is below $0.01$ --- and both hold at every one of the
$70$ scored cells. The worst difference over those cells is
$2.9766\cdot10^{-3}$.

\emph{Every set attempted is reported, including those that resolve
nothing.} Cells with $n_c<1000$ are printed with their counts and
excluded from scoring rather than dropped; there are eight, and one of
them is \emph{this} partition's depth~$5$ at $n_c=67$. So the sweep does
not score the cell on which Measurement~\ref{meas:maincoef} leans
hardest, and what it establishes is transfer across shapes, not a second
verdict on that cell.

\textbf{All $70$ scored differences have the same sign}: the main term
exceeds the exact value at every cell of every set. That is consistent
with the decomposition above, in which the divisor deficit enters
negatively, and it is a statement about these $70$ cells and not a proof
that the sign cannot change.

The run's first block is a check on its own instrument rather than on
the paper. $E_{\mathrm{exact},c}[A_S]$ is built from the residues alone
--- $f(q)=\frac{q-2}{q-1}+\frac1{q-2}$ on primes dividing neither of a
pair, $g(q)=\frac{q-1}{q-2}$ on primes dividing both, weighted
$m_im_j$ over classes of equal depth --- and at $S=(3,5,7)$ it returns
$\tfrac{403}{240}$, $\tfrac{941}{605}$, $\tfrac{31}{15}$ and
$\tfrac{16}{5}$ with residue counts $48,44,12,1$, which are the
rationals Measurement~\ref{meas:secondcell} uses. Its $S=(3,5,7)$
column returns $0.295707753$, $0.126454301$, $0.825436443$ and
$2.374142099$, the values that measurement prints. \textbf{Those numbers
were derived rather than read}, so the agreement is between two
constructions and not within one.

The convention is $D'_c$ throughout, the form the registration fixed,
and not \eqref{eq:Dc}; the two differ by $E_\infty-E_{\mathrm{all},B}$
as recorded above.
\end{measurement}
...
divisor-sampling term's \emph{rate} does not vary with the depth: it is
$O_\eta(B^{\eta}/\ell)$ with $\ell=B/2Q$, a band parameter, so one bound
covers every cell of an octave. That the bound is uniform does not make
the term itself constant across cells; what is fixed is the rate, and
the constant in front of it is a separate question this does not settle.
...
each octave is unquantified, so it excludes nothing the measurement
does not already exclude. \textbf{The proof's bound is weaker than the
measurement at every cell.} It does
...
reads is large only where $D_c$ is small. At ten times the sample the two
...
Supplement~S2 gives both readings and the verdicts each produces.
...
estimator was reading rather than adding to. Supplement~S2 records the
...
excess does not decay to zero asymptotically. The decay exponent of the effect itself
...
and they are computed from explicit finite correlations and weighted
inner products.

% CLAIM:C5
\item Observation~\ref{obs:coh} and Note~\ref{note:notanerrorbar}:
over the eight measured octaves that quantity is consistent with
$(\log N)^{-1/2}$ at the shallow cells, where
Note~\ref{note:threeterms}'s fraction is constant, and is inconsistent
with count-based $n_c^{-1/2}$ scaling over the same range. The exponent
itself is measured at all six
...
same measurement computes $D_c$ exhaustively, by one autocorrelation per
...
on the estimator the run registered; Supplement~S2 gives them. It is a computed component of the proposition's own error, not a failure of it.
...
computed from the arithmetic alone}: Lemma~\ref{lem:cellmom} also
...
% CLAIM:C6,C2
\item Two things are needed for the floor, and only the second is what
the placebo removes: coherence of $u_c(v)$ across the cell, and a cell
correlated with the arithmetic that coherence comes from. Nothing here
shows that pair is special to this field, and nothing here shows it is
not. Sharing the arithmetic input is
...
% CLAIM:none
All registered checks, including the failed ones and the one
recorded void, are reported with their verdicts in Supplement~S2
(Appendix~\ref{sec:repro}). \emph{Registered} and not
\emph{preregistered}: each rule was written into the run that scores it,
and the packet establishes no ordering between a rule and its own
measurement --- Supplement~S2 names the one block whose heading claims
more than that. Several numerical patterns conjectured in
...
the directory \texttt{P4-coherent-cell-floor-r1/}. The figures quoted
are pinned by the SHA-256 list in that directory, as
Appendix~\ref{sec:repro} sets out. No data derived
...
$\theta(x)\sim x$ and is the only analytic input the second form adds
to Mirsky's theorem.
\end{proof}
...
\noindent in the directory \texttt{P4-coherent-cell-floor-r1/}. That
address is mutable, so what pins the bytes travels with them:
\texttt{PACKET.json} in that directory carries a SHA-256 for every file
in \texttt{code/} and \texttt{results/}, and a figure can be checked
against the exact source that produced it without resolving a tag or
trusting the branch. The pin is on those and not on this document,
which cannot cite a hash that covers it. Each entry
...
behaviour and not a failure: a run whose own registered rule was
refuted says so in its exit code as well as in its output, and the
refutations are reported in Supplement~S2.

The Lean development is in \texttt{P4-coherent-cell-floor-r1/lean/}, with
...
\texttt{lab\_cellset\_sweep.py} & \texttt{lab\_cellset\_sweep.txt}
 & Measurement~\ref{meas:sweep}\\
...
its verdict, including the thirteen the eleven named runs did not meet, the
fourteenth recorded in a separate output, and the one recorded void. The packet preserves the second-cell prediction and
measurement artifacts, but does not make a claim about their historical
ordering.
...
Lemma~\ref{lem:envtv}, Proposition~\ref{prop:scaleinv} and
Proposition~\ref{prop:V} are proved and use no computation; those
seven are every proof in the paper.
...
the last run of the table above. Counting a named statement as
...
\subsection*{Supplement S2: the registered rules and their verdicts}

Six of the eleven named runs record a rule their own output did not meet.
The separately retained first second-cell output also records P1 as
refuted. The
...
\texttt{audit\_cn\_coin\_deep}. Under that unit forty rules
carry a verdict --- twenty-six hold, thirteen refuted, one void ---
and one further label, W3, sits under the same registration heading as
W1, W2 and W4 and is reported without a verdict, so forty-one labels
are registered in all. The eleventh run,
\texttt{lab\_cellset\_sweep}, carries two registered rules and both
hold; its \texttt{C1} and \texttt{R3} are checks on its own instrument
rather than registered rules, and its output says so on a separate line
from the two, because a count that mixes the two answers a different
question from the one asked here. The fourteen are named here because a
...
$\mathrm{se}_c\sim N^{-b}$ over the eight octaves, and the quantity
...
Seven of the fourteen concern a prediction about a row this paper
...
from the one it appears to ask. P1 is the omitted fourteenth: the first
second-cell run scored one sampled draw at three depths, and its
depth-$2$ draw missed the registered three-standard-error cap. Its result
file records it as refuted; it is the separate output noted above, not
one of the eleven named runs. Nothing above rests on that sampled rule.
The other three did more than that, and are listed here because the count alone would
