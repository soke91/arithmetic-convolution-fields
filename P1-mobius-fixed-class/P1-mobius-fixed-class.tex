...
sufficiently large even integers from a M\"obius-twisted
Elliott--Halberstam hypothesis.  In their
Section~3 that hypothesis is used to control two signed fixed-class terms,
...
$E_3(\alpha)\ll_A N(\log N)^{-A}$ used in the Huang--Li argument is,
at the same error scale, equivalent to their equation~(22).  Together
with the subsequent bound for the remaining correlation used by
Huang--Li, this relation yields binary Goldbach for sufficiently large
even $N$.  Under that estimate,
...
$\mu*\log=\Lambda$ and reproduces $\widetilde r(N)$.  We do not claim
...
Using \eqref{eq:ctrunc} and $\Tcut_m(t)\le N$, the two halves are
\[
...
\]
Thus both are $\ll Ne^{-c\sqrt{\log N}}$ for every fixed $c<1$.
...
\end{equation}
the binary Goldbach sum itself.
...
$1-\AAA(N)\gg(\log N)^{-2}$.  Choosing the error exponents in (22)
and in $U(N)$ larger than $2$ therefore makes $\tilde r(N)>0$ for all
sufficiently large even $N$.  This is the binary Goldbach implication
used here.  Moreover, since
...
all sufficiently large $N$.  Thus $\widetilde r(N)>0$.
\end{proof}
...
\]
Equation~\eqref{eq:nolog} therefore implies $\widetilde r(N)>0$ for
all sufficiently large even $N$.
\end{proof}
...
all; the resulting $+1$ then contributes $\alpha\sqrt N\log N$.
That crude estimate would therefore require $\alpha<\sqrt N$; the
...
Press, 2025. The Bombieri--Vinogradov theorem used here is \S20.2,
Theorem~20.2.
...
drafting and computational-audit details are reported in the Supplementary
Material; no computation is used as a premise of a proof.

\paragraph{Data availability statement.} The code, stored outputs, and Lean~4 development are openly available at
\url{https://github.com/soke91/arithmetic-convolution-fields}, in the
directory \texttt{P1-mobius-fixed-class/}. The Supplementary Material
describes the audit scripts and the numerical checks. No data derived from
human or animal subjects were used.
