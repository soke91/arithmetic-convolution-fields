...
into cells indexed by how many of $3,5,7,11,13$ divide $N$. Asking
whether a cell mean departs from the band mean needs a reference scale
for that contrast.

We compute the scale exactly. Under the null replacing the $\mu(v)$ on
their own support by independent Rademacher signs, the variance of any
cell contrast is a doubly centred quadratic form in the field's
covariance kernel, and reduces to three convolutions
(Lemma~\ref{lem:cellmom}). That the independent-sample formula
$\mathrm{sd}/\sqrt{n_c}$ fails for correlated observations is standard;
what is not is that here the kernel is exactly computable, that
divisibility cells align with it, and that a uniformly random cell of
the same size recovers the finite-population formula exactly
(Lemma~\ref{lem:permfloor}) --- so the count-based scale is right for a
random cell and wrong for these.

Measured over eight octaves to $1.6\cdot10^{7}$, the exact null
standard deviation of a cell contrast shrinks by $1.21$ where a count
predicts $11.3$, and under a label permutation preserving every cell
size it collapses by factors $3.8$ to $105$. Its size is predicted with
no fitted parameter by the excess of the shift's singular series over
same-cell pairs (Proposition~\ref{prop:scaleinv}), whose main term is
computed here in exact rational arithmetic and confirmed on a second
cell partition against predictions sealed before that partition was
measured.

No distributional law for $Z$ is asserted, and no Goldbach consequence
is drawn.
\end{abstract}
...
\item \textbf{The statements here are not equally reviewed, and the
uneven ones are named.} Material carried forward from the first
version of this programme has been through three independent
adversarial reviews in fresh context; statements first written later
have had fewer. Nothing shipped here records those reviews, so that
count stands on the same footing as the fraction below it. Among the latter are the derivation of
Observation~\ref{obs:coh} and the statement of
Lemma~\ref{lem:placebo}. Reviews of this kind have caught, in this
project's own record, roughly half of what a later review then found;
that is an impression from the running tally and not a figure measured
by anything shipped here. Read it only as the reason nothing here
should be taken as certified, and those two least of all.
\end{itemize}
...
in $0.507,\,0.041,\,0.519,\,0.275,\,0.992,\,2.025$. Only the cell of
three elements clears the bar, and clearing it is not agreement. The
precision is set by the draw count alone and is therefore the same at
all six cells; what differs is the deviation, and the only one that
clears the common precision is the one where the simulation and the
closed form disagree by more than the simulation's own noise --- by
$2.0$ standard errors, at $n_c=3$. That is one deviation
...
registered in the result file score the $60$-draw quotation this
paper no longer makes; the ratios of this paragraph are checked in a
...
there, which is the one step taken here. \textbf{An interval built from a count is therefore a factor
...
cells of a standardised deviation against an exactly computed floor,
...
enumeration, so what the true value's rank supports is $p\le1/11$ and
no more; the enumeration of Measurement~\ref{meas:Dc} below is the
...
$0.991676$. \textbf{So the count-based scale is right for a random
cell and wrong for the arithmetic cells cut here} --- the thesis of
this paper is a statement about which cells, not about counts. The
...
and the step it puts between depths~$4$ and~$5$, $+0.004706$, exceeds
the whole printed rise there, $+0.00336$.
...
\label{prop:scaleinv}
Depth fixes how many of $3,5,7,11,13$ divide $N$ and not the residues
themselves, so a cell is a union of divisibility patterns. $D_c$
depends, once the primes above $13$ are shown to contribute the same
factor to both averages, only on the law of the shift $h$ induced by
those patterns and on the densities with which they enter the cell.
That the primes above $13$ do so is the hardest step of the proof and
not a hypothesis readable off the construction. Both are the same at
every scale, so $D_c$ is scale-invariant up to what the proof
carries: the cell's own resolution, which is $O(1/\ell)$ with
$\ell=B/2Q$ and governs the equidistribution of the residues and the
sampling of the divisors alike, and $O_\eta(B^{\eta-1})$ from the
truncations. Both vanish with the band, and the proposition predicts
a fitted exponent of zero at every depth whose main term does not
vanish.

The implied constants are absolute except for the dependence on $\eta$
that the subscript records. In particular they do not depend on the
cell $c$, on its depth, on its size $n_c$, or on $B$; the parameter set
$\{3,5,7,11,13\}$ enters only through $Q$, fixed throughout. The
$\eta$-dependence is not uniform as $\eta\to0$ --- the constant comes
from the Euler products of Rankin's bound below and blows up there ---
so $\eta$ is fixed in advance and the statement is one about each fixed
$\eta>0$ separately.
\end{proposition}
...
$O(1/\ell)$ per class, the residues at the five primes are independent within a
pattern, so the joint law modulo their product is determined and not
only the five marginals. The weights with
...
It is not enough that each prime above $13$ is separately
equidistributed: $\SS_2$ reads them all at once, and the primes
comparable to the band length are equidistributed modulo nothing.
...
same constant in both averages, and it leaves the difference:
\[
  D_c=2C_2\prod_{p>13}\Bigl(1+\frac1{p(p-2)}\Bigr)
  \bigl(E_{\mathrm{same},c}[A]-E_{\mathrm{all}}[A]\bigr)
  +\mathcal E.
\]
Both $A$-averages are fixed by the pattern weights, which are the same
at every scale. Hence \eqref{eq:Dc} converges to a constant depending
only on the cell, and its scale exponent is $0$ \emph{at every depth
at which that difference does not vanish}. Convergence is additive and
an exponent is multiplicative: $\mathrm{d}\log D_c/\mathrm{d}\log B\to0$
needs the main term to be nonzero, and where
$E_{\mathrm{same},c}[A]=E_{\mathrm{all}}[A]$ the proposition says only
that $D_c\to0$ and predicts no exponent at all. That difference is a
...
that measurement finds. \emph{So the largest of the six spreads is not
...
derivative. Writing $D_c(B)$ for the value of \eqref{eq:Dc} computed in
the band of length $B$, the one-step exponent is
\[
  e_c(B)\;=\;-\,\frac{\log\bigl|D_c(2B)/D_c(B)\bigr|}{\log 2},
\]
the sign being fixed by the convention $|D_c|\sim B^{-e}$, so that a
positive $e$ means a decaying $D_c$. The $e$ tabulated below is minus
the least-squares slope of $\log|D_c|$ against $\log B$ over the three
...
\emph{As registered} is a different verdict, and this paper has
called the code's the registered one. The registration reads
``exceeds $0.02$ at any depth \emph{by more than three sampling
standard errors}''; the code applies the flat $0.02$ with no
allowance. Scored as the registration reads, the allowance at
depth~$1$ is $0.1232$ against a spread of $0.0902$, and \textbf{M2
holds at all six depths}. M4's rule carries no allowance --- refuted
if $|e|\ge0.01$ at any depth --- so M4 is refused at depths~$0$
and~$1$ under either reading. The code is the stricter of the two and
its verdict is the one this run gates on; which of them may be called
\emph{as registered} is the registration's.
...
estimator and not of $D_c$. The registered verdicts stand as scored:
they were scored on the estimator this file registered, and this
paragraph does not rescore them. What it separates is the estimator
from the quantity. The deepest cell keeps the largest spread,
$1.28$ per cent, which Measurement~\ref{meas:maincoef} computes as
$1.24$ --- the divisor deficit of a cell of a few
tens of elements and is the only one of the six that the estimator was
reading rather than adding to. M2's two per cent is printed in the result file, beside
the remark that the registration had allowed three sampling standard
errors and the code applied the flat band instead; the depth-wise
spreads are printed there too, so a reader who opens only the result
can reproduce that refusal. The exact-$D_c$ block quoted above puts
M4's $|e|<0.01$ into that file as well, so that refusal is
reproducible from the result too. The declared field's figures are the ones printed
above; the ten-times-harder figures are a separate resolution and
are not mixed into the table.
...
$128$ in $N$, and absolutely too narrow by factors of $7.3$ to $158$ at
...
decay. Proved; and refused at depth~$1$ by both rules and at
depth~$0$ by the exponent rule alone, as the code scores them. Two
things narrow that. Scored as the registration reads --- $0.02$ plus
three sampling standard errors --- M2 holds at all six depths, and
only M4's refusals survive, its rule carrying no allowance. And the
refusals that survive are the sampling estimator's. $D_c$ is deterministic and the same measurement
computes it over all pairs by one autocorrelation per cell; done that
way the spread runs $0.0001$ to $0.0128$ across the six depths and the
fitted exponent $-0.000059$ to $-0.009202$, so all six sit inside both
registered caps, the shallow four by three orders. The registered
verdicts stand as scored --- they were scored on the estimator the run
registered --- and what the exact figures separate is the estimator
from the quantity. The largest of them is the deepest cell's $1.28$
per cent, and Measurement~\ref{meas:maincoef} computes it: $1.24$ per
cent from the divisor deficit of a cell that is a single residue
class. It is the proposition's own error term, not a failure of it.
...
$k$-tuple conjecture}, arXiv:1004.1084 [math.NT]. Equation~(1.5)
states Gallagher's result as
$\sum_{|H|=k,\,H\subset[1,H]}\SS(H)\sim H^{k}$, whose $k=1$ case is
the average $1$ over all shifts used above; \S1 also records
$\SS(\mathcal H\cup\{h\})=0$ for odd $h$, which is the vanishing that
makes the average over even shifts twice that.
...
uniform-in-$x$ partial summation: $(\log p)^2=(1+o(1))(\log N)^2$ for
$p>N^{1-\varepsilon}$, while $\pi(N^{1-\varepsilon})=o(N/\log N)$
...
\noindent in the directory \texttt{P4-coherent-cell-floor/}. Each entry
...
takes full responsibility for everything stated here. Because that
division of labour makes independent checking the material question,
this note says exactly what was checked and by what, rather than
asserting that it was checked.

\emph{Machine-verified.} Lemma~\ref{lem:cellmom}'s algebraic
identity, Lemma~\ref{lem:permfloor}'s closed form, and
Lemma~\ref{lem:placebo} are proved in Lean~4 against Mathlib, with the
axiom report reproduced in the code packet; a proof there depends on
\texttt{propext}, \texttt{Classical.choice} and \texttt{Quot.sound}
and on nothing else. \textbf{Proposition~\ref{prop:scaleinv} is not
machine-verified.} Its statement is written out in Lean and left
unproved. The finite half of its argument --- Rankin's truncation step
both ways, the exact count of one residue class in an interval, and
the cancellation of the class count $m$ --- is verified separately;
the analytic half, the Euler product and its convergence, is not. The
three errors this proposition's proof has carried were all in the
finite half, which is why that half was the one formalised.
...
figure that no cited run prints. Every registered rule is reported
with its verdict, including the fourteen the runs did not meet and the
one recorded void. The predictions of
Measurement~\ref{meas:secondcell} were committed to the repository
before the measurement that scores them existed.
...
findings, and several statements in this paper are corrections those
readings produced. That is a check on internal consistency, on
arithmetic, and on whether each statement matches the run it cites.
\textbf{It is not a reading by a subject expert}, and
Proposition~\ref{prop:scaleinv} has not had one.
...
heuristic $0.049540$. Nothing in this paragraph is referred to by its
position: an insertion into that list moved two items once already,
and an ordinal for a paragraph of Measurement~\ref{meas:mc} pointed at
the wrong one until it was named instead;
...
this sentence has been wrong four times: two earlier printings said so
themselves, a third read five for the second file, and a fourth made
the first eight by counting the items of \S\ref{sec:notclaimed} and the
paragraphs that carry no figure of their own alongside the named
statements. Under a unit that can be checked the two numbers are five
and six.
...
two of those four score a $60$-draw quotation an earlier printing of
this work made and this paper does not: the table above is at $2000$
draws, and the precision Measurement~\ref{meas:mc} reasons from is
computed in the text rather than gated by any rule of that file. Its
cleanliness is against the earlier quotation. The ratios that
measurement now states are checked in a block the file marks post
hoc, and one of them --- depth~$4$ at $0.992$ of the precision ---
is what that block was added to make visible.
...
paper prints --- three times, once early, once in
Note~\ref{note:notanerrorbar} and once at the close --- is
...
Note~\ref{note:collapse} all follow from it. Two earlier printings of
this paragraph were wrong about this count in the same direction: one
said none of the ten changed anything, the next said one did.
