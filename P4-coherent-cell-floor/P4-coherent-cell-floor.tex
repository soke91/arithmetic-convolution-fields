...
$3.8$ to $105$. For its size we offer a mechanism found the same way:
the excess of the shift's singular series over same-cell pairs, whose
main term is a rational number and carries no fitted parameter. On the
partition that suggested it the agreement is \emph{post hoc}, and one
rule registered there was refuted. The mechanism was then written down
for a second, differently shaped partition and its predictions
committed before that partition was measured; the exact measurement
falls inside the registered interval at every cell. That transfer, not
the fit on the first partition, is the evidence offered.
