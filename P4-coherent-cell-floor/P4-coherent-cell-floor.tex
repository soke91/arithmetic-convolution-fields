...
(Lemma~\ref{lem:cellmom}). That $\mathrm{sd}/\sqrt{n_c}$ fails for
correlated observations is standard; what is not is that here the
kernel is exactly computable, that divisibility cells align with it,
and that a uniformly random cell of the same size recovers the
finite-population formula exactly (Lemma~\ref{lem:permfloor}) --- so
the count-based scale is right for a random cell and wrong for these.
...
For its size we offer a mechanism: the excess of the shift's singular
series over same-cell pairs, whose main term is rational and carries no
fitted parameter. On the partition that motivated it the agreement is
\emph{post hoc} and one rule registered there was refuted. The
mechanism was then written down for a second, differently shaped
partition and its predictions committed before that partition was
measured; the exact measurement falls inside the registered interval at
every cell. That transfer, not the fit on the first partition, is the
evidence offered.

No distributional law for $Z$ is asserted, no Goldbach consequence is
drawn, and the decay of the effect with $N$ is not determined here.
\end{abstract}
...
\label{prop:scaleinv}
We establish an asymptotic statement and not an identity: $D_c$ agrees
with a quantity determined by the cell alone, up to error terms that
vanish with the band. Nothing below asserts equality at a fixed band,
and no rate is claimed for the agreement.
...
complete in \S\ref{sec:size}. It is the one labelled statement here
