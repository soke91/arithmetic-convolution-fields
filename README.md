...
Everything for it is in `P4-coherent-cell-floor/`: the source, the
compiled PDF, the code, the output that code produced, and the Lean
development. One directory per paper; this repository holds nothing
else.
...
`P4-coherent-cell-floor/lean/` holds the formalisation, and its
`axioms.txt` is the axiom report the paper refers to, produced by
